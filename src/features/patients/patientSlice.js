...
async ()=>{
  const response = await axios.get('https://hospitalmanagement.rohitmane2.repl.co/patients');
  return response?.data?.data
}
)
export const addPatients= createAsyncThunk('/patients/addPatients', async(bodyData)=>{
  const  response = await axios.post('https://hospitalmanagement.rohitmane2.repl.co/patients', bodyData);
  return response.data.data;
})
export const deletePatientData = createAsyncThunk('/patients/deletePatient', async(id)=>{
const response = await axios.delete(`https://hospitalmanagement.rohitmane2.repl.co/patients/${id}`)
return response.data.data
})
export const updatePatients = createAsyncThunk('/wards/updatePatients', async({id, formData})=>{
  const response = await axios.put(`https://hospitalmanagement.rohitmane2.repl.co/patients/${id}`, formData)
 return response.data.data
})
