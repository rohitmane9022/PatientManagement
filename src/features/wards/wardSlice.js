...
async ()=>{
  const response = await axios.get('https://hospitalmanagement.rohitmane2.repl.co/wards');
  return response?.data?.data
}
);
export const addWards= createAsyncThunk('/wards/addWards', async(bodyData)=>{
  const  response = await axios.post('https://hospitalmanagement.rohitmane2.repl.co/wards', bodyData);
  return response.data.data;
})
...
export const updateWards = createAsyncThunk('/wards/updateWards', async({id, formData})=>{
  const response = await axios.put(`https://hospitalmanagement.rohitmane2.repl.co/wards/${id}`, formData)
 return response.data.data
})
